...
const SelectOptionPicker = () => {
    ...
    const selectOptionRef = useRef(new Animated.Value(0)).current;
    
     useEffect(() => {
        if (isModalVisible) {
            Animated.timing(selectOptionRef, {
                toValue:1,
                duration: 500,
    ...
    const bottomTranslateY = selectOptionRef.interpolate({
        inputRange: [0, 1],
    ...
    const hospitalServiceDB = ["Government Center", "Semi-Government", "Private Center", "Other"];
